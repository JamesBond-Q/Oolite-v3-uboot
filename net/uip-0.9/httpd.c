...
extern const struct fsdata_file file_flashingfw_html;
	...
	if (start) {
		...
		if (end) {
			...
			if ((end - (char *) uip_appdata) < uip_len) {
				...
				// size check
				if ((webfailsafe_upgrade_type == WEBFAILSAFE_UPGRADE_TYPE_UBOOT || webfailsafe_upgrade_type == WEBFAILSAFE_UPGRADE_TYPE_ART) && (hs->upload_total != WEBFAILSAFE_UPLOAD_UBOOT_SIZE_IN_BYTES)) {
					printf("## Error: wrong file size, should be: %d bytes!\n", WEBFAILSAFE_UPLOAD_UBOOT_SIZE_IN_BYTES);
					webfailsafe_upload_failed = 1;
				} else if(hs->upload_total > (info->size - 196608)) {
			...
			printf("## Error: couldn't find data start!\n");
		}
				...
				if (hs->state == STATE_FILE_REQUEST) {
					...
					if (start) {
						...
						if (end) {
							...
							// TODO: sizeof(char) always gives 1?
					...
					if (hs->upload >= hs->upload_total) {
						...
						if (!webfailsafe_upload_failed) {
							...
							if(webfailsafe_upgrade_type == WEBFAILSAFE_UPGRADE_TYPE_FIRMWARE){
								fs_open(file_flashingfw_html.name, &fsfile);
							} else {
