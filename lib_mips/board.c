...
#define SEL_LOAD_ART_WRITE_FLASH		5
#define SEL_ENTER_WEB_SAFE_HTTPD       	6
...
//+++by bond
	printf("   %d: Load ART file then write to Flash via TFTP!!! \n", SEL_LOAD_ART_WRITE_FLASH);//"5"
	printf("   %d: Enter to web failsafe httpd!!! \n", SEL_ENTER_WEB_SAFE_HTTPD);//"6"
	printf("   %d: Load Boot code to SDRAM via TFTP. \n", SEL_LOAD_BOOT_SDRAM);//"8"
//---by bond
#ifdef RALINK_UPGRADE_BY_SERIAL
	printf("   %d: Load Boot Loader code then write to Flash via Serial. \n", SEL_LOAD_BOOT_WRITE_FLASH_BY_SERIAL);//"7"
#endif // RALINK_UPGRADE_BY_SERIAL //
	printf("   %d: Load Boot Loader code then write to Flash via TFTP. \n", SEL_LOAD_BOOT_WRITE_FLASH);//'"9"
}
...
int tftp_config(int type, char *argv[])
{
	...
	//+++by bond
	else if(type == SEL_LOAD_ART_WRITE_FLASH){
		argv[1] = "0x80100000";
		printf("\tInput ART filename ");
		strncpy(argv[2], "art", ARGV_LEN);
	}
	//---by bond
	s = getenv("bootfile");
	if (s != NULL) {
...
//+++by bond
#if defined(MT7628_ASIC_BOARD)
	/*Enable 32MB/64MB SPI FLASH*/
	unsigned long chip_mode, data;
	...
	//RALINK_REG(RALINK_SYSCTL_BASE+0x10) |= (0x7<<1);
	RALINK_REG(RALINK_SYSCTL_BASE+0x10)&= ~(0x7<<1);
	//RALINK_REG(RALINK_SYSCTL_BASE+0x10) = 0x144146;
	data = RALINK_REG(RALINK_SYSCTL_BASE+0x10);
	printf("**********After seting:SYSCFG0 = 0x%02x\n",data);
...
//+++by bond
	...
	printf("\n============================================ \n");
	printf("\n########## WEB FAILSAFE MODE ########### \n\n");
	printf( "Press WPS button for more than 2 seconds to run web failsafe mode\n" );
	...
	if ( counter > 0 ) {
		...
		printf( "\n GPIO_38 was pressed for %d seconds\nHTTP server is starting for firmware update...\n", counter );		
		printf("============================================ \n\n");
		eth_initialize(gd->bd);
		NetLoopHttpd();
	} else {
		printf( "\nCatution: GPIO_38 wasn't pressed or not long enough!\nContinuing normal boot...\n" );		
		printf("============================================ \n\n");
	}
...
//+++by bond
	...
	printf("\n============================================ \n");
	printf("\n########## FAILSAFE MODE ########### \n\n");
	printf( "1.Press GPIO_38  for more than 5 seconds \n  to run web failsafe mode\n" );
	printf( "2.Press GPIO_38  for more than 1 seconds \n  and less than 3 seconds to run gpio test mode\n" );
	printf( "GPIO_38 is pressed for: %2d second(s)", counter );
	...
	while( DETECT_WPS() ) {//pressed the button
	...
	if ( counter >= 5 ) {
		...
		printf( "\n 1.GPIO_38 was pressed for %d seconds\nHTTP server is starting for firmware update...\n", counter ); 	
		printf("============================================ \n\n");
		eth_initialize(gd->bd);
		NetLoopHttpd();
	} else if(counter >= 1 && counter <= 3) {
		//printf( "\nCatution: GPIO_38 wasn't pressed or not long enough!\nContinuing normal boot...\n" ); 
		printf("\n2.Enter the gpio test mode!!!\n");
		printf("============================================ \n\n");
...
*/
		...
		udelay(1000*1000);//Must have this delay!!! For disappears shakes!!
...
//+++by bond
		case '5':
			printf("   \n%d: System Load system code then write to Flash via TFTP!!!\n", SEL_LOAD_ART_WRITE_FLASH);
			printf(" Warning!! Erase ART in Flash then burn new one. Are you sure?(Y/N)\n");
			confirm = getc();
			if (confirm != 'y' && confirm != 'Y') {
				printf(" Operation terminated\n");
					break;
			}
			tftp_config(SEL_LOAD_ART_WRITE_FLASH, argv);
			argc= 3;
			setenv("autostart", "no");
			do_tftpb(cmdtp, 0, argc, argv);
			if (NetBootFileXferSize > CFG_FACTORY_SIZE) {
				printf("Abort: art file size %d too big! \n", NetBootFileXferSize);
			}

			if (1) {
				unsigned int load_address = simple_strtoul(argv[1], NULL, 16);//argv[1] = "0x80100000";
				printf("Erase Then Flash the ART!!\n");
				raspi_erase_write((u8 *)load_address, CFG_KERN_ADDR-CFG_FLASH_BASE-CFG_FACTORY_SIZE, NetBootFileXferSize);
			}
			//reset            
			do_reset(cmdtp, 0, argc, argv);
			break;
		...
		case '6':
			printf("\n%d: Enter to web failsafe httpd! \n", SEL_ENTER_WEB_SAFE_HTTPD);
