...
static struct chip_info chips_data [] = {
	...
	{ "W25Q256FV",      0xef, 0x40190000, 64 * 1024, 512, 1 },	//+++by bond
...
int do_mem_cp(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	else if (!strncmp(argv[0], "cp.arttoddr", 12)) {
		src = 0xbc040000;
		dest = 0x80100000;
		count = 0x10000;
		printf("\n Copy art from Flash[0x%08X byte] to DDR[0x%08X],size[%d byte].... \n", src, dest,count);
		//flash_sect_protect(0, CFG_FLASH_BASE, CFG_FLASH_BASE+CFG_BOOTLOADER_SIZE-1);//(0.0xbc000000,0xbc000000+0x30000-1)
		//raspi_erase_write((char *)addr, dest, count);
		//raspi_read(char * buf,unsigned int from,int len);
		memcpy((u8 *)dest,(u8 *)src,count);
		return 0;
	}
	else if (!strncmp(argv[0], "cp.ddrtoart", 12)) {
		//addr = 0x80040000;
		addr = 0x80100000;
		dest = 0x40000;
		ulong count = 0x10000;
		printf("\n Copy art from Flash[0x%08X byte] to DDR[0x%08X],size[%d byte].... \n", addr, dest,count);
		//unsigned int load_address = simple_strtoul(addr, NULL, 16);
		//raspi_erase_write((char *)addr, dest, count);
		
		//unsigned int load_address = simple_strtoul(addr, NULL, 16);//argv[1] = "0x80100000";
		raspi_erase_write((u8 *)addr,dest, count);
