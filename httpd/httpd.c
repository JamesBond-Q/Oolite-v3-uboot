...
*/
			...
			if(hs->upload_total >= 102400 && hs->upload_total <= 200000){
				webfailsafe_upgrade_type = WEBFAILSAFE_UPGRADE_TYPE_UBOOT;
				printf( "Upgrade type: uboot!!!\n" );

			} else if(hs->upload_total >= 2883588 && hs->upload_total <=16580608){
				webfailsafe_upgrade_type = WEBFAILSAFE_UPGRADE_TYPE_FIRMWARE;
				printf( "Upgrade type: firmware!!!\n" );
				//printf("monster file!!what are you doing?\n");
				//webfailsafe_upload_failed = 1;
